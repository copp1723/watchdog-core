...
def test_cost_per_sale_by_vendor_headline() -> None:
    out = cost_per_sale_by_vendor(df)
    keys = ['worst_vendor', 'worst_cps', 'best_vendor', 'best_cps', 'shift_pct']
    if isinstance(out['headline'], dict):
        for k in keys:
            assert k in out['headline']

def test_sales_by_salesperson_headline() -> None:
...
def test_new_vs_used_headline() -> None:
    out = new_vs_used(df)
