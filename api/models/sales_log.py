...
class SalesLogRow(BaseModel):
    ...
    profit: Optional[float] = Field(
        None, description="Profit from the sale (may be same as gross)"
    )
    listing_price: Optional[float] = Field(None, description="Original asking price")
    expense: Optional[float] = Field(
        None, description="Marketing or lead acquisition cost"
    )
    discount: Optional[float] = Field(
        None, description="Difference between listing and sold price"
    )
    days_to_close: Optional[int] = Field(
        None, description="Number of days from lead to sale"
    )
    
    # Categorical fields
    lead_source: Optional[str] = Field(
        None, description="Source of the lead (marketing channel)"
    )
    ...
    is_new: Optional[bool] = Field(
        None, description="Whether the vehicle was new or used"
    )
    
    # Derived financial metrics
    margin_pct: Optional[float] = Field(
        None, description="Profit as percentage of sale price"
    )
